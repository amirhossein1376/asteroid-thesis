...
#Here. I'm gonna try to train a simple DPT model on Libri2Mix clean part to see the results
...
tag="try-8"  # Controls the directory name associated to the experiment
...
if [[ $stage -le 2 ]]; then
  ...
  mkdir -p logs
  CUDA_VISIBLE_DEVICES=$id $python_path train-8.py --exp_dir $expdir \
		--epochs $epochs \
