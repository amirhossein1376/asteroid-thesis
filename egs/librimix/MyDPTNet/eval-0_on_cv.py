...
from asteroid.data import MyCommonMix
...
parser.add_argument(
    "--base_dir", type=str, required=True, help="Base Directory For Files"
)
parser.add_argument(
    "--metadata_dir", type=str, required=True, help="The subdirectory of the base directory in which the madata files have been stored"
)
parser.add_argument(
    "--test_segment", type=str, required=True, help="The subdirectory of the base directory in which the test files have been stored"
)
parser.add_argument(
    "--task",
    type=str,
...
ASR_MODEL_PATH = (
    ...
    # "/home/user01/codes/model/checkpoint-166747"
...
def main(conf):
    ...
    anno_df = pd.read_csv(Path("/mnt/new_drive/datasets/cv-corpus-6.1-2020-12-11/fa/our_data/raw/test_annotations.csv"))
    wer_tracker = (
        MockWERTracker() if not conf["compute_wer"] else WERTrackerWavLM(ASR_MODEL_PATH, anno_df, conf["sample_rate"], process_before_transcription=True, use_gpu=False)
    )
    ...
    test_set = MyCommonMix(
        base_dir=conf["base_dir"],
        metadata_dir=conf["metadata_dir"],
        part=conf["test_segment"],
        task=conf["task"],
    ...
    for idx in tqdm(range(len(test_set))):
        ...
        transcriptions, dic = wer_tracker(
                                mix=mix_np,
                                clean=sources_np,
