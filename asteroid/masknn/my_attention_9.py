...
class DP9Transformer(nn.Module):
    ...
    def __init__(
        ...
        super(DP9Transformer, self).__init__()
    ...
    def forward(self, mixture_w):
        ...
        for layer_idx in range(len(self.layers)):
            intra, inter, net_lin_proj = self.layers[layer_idx]
            mixture_w1 = self.ola.intra_process(mixture_w, intra)
            mixture_w2 = self.ola.inter_process(mixture_w, inter)
            mixture_w = torch.stack((mixture_w1, mixture_w2), dim=dim)
            mixture_w = net_lin_proj(mixture_w)
            mixture_w = mixture_w.squeeze(dim=dim)
