...
class MyWavLMLibriMix(Dataset):
    """Dataset class for LibriMix source separation tasks.

    Args:
        base_dir (str): The path to the librimix dataset ( a specific configuration of mode and sample rate)
        metadata_dir (str): The path to the metadata folder.
        part (str): A string containing the part of dataset we want to work on
        ...
        processor (huggingface model) : The model to extract wavlm features
    ...
    def __init__(
        self, base_dir, metadata_dir, part, task="sep_clean", sample_rate=16000, n_src=2, segment=3, return_id=False, processor=None, process_sources=False
    ):
        ...
        self.process_sources = process_sources

        self.csv_dir = os.path.join(self.base_dir, self.metadata_dir)
        # Get the csv corresponding to the task
        if task == "enh_single":
            md_file = [f for f in os.listdir(self.csv_dir) if "single" in f and "mixture" in f and part in f][0]
            self.csv_path = os.path.join(self.csv_dir, md_file)
        elif task == "enh_both":
            md_file = [f for f in os.listdir(self.csv_dir) if "both" in f and "mixture" in f and part in f][0]
            self.csv_path = os.path.join(self.csv_dir, md_file)
            md_clean_file = [f for f in os.listdir(self.csv_dir) if "clean" in f][0]
            self.df_clean = pd.read_csv(os.path.join(self.csv_dir, md_clean_file))
        elif task == "sep_clean":
            md_file = [f for f in os.listdir(self.csv_dir) if "clean" in f and "mixture" in f and part in f][0]
            self.csv_path = os.path.join(self.csv_dir, md_file)
        elif task == "sep_noisy":
            md_file = [f for f in os.listdir(self.csv_dir) if "both" in f and "mixture" in f and part in f][0]
            self.csv_path = os.path.join(self.csv_dir, md_file)
    ...
    def __getitem__(self, idx):
        ...
        if "enh_both" in self.task:
            ...
            if self.process_sources:
                s = torch.from_numpy(s)
                s = self.processor(s, sampling_rate=self.sample_rate).input_values[0]
        ...
        # sources_list = [np.pad(x, (0, self.seg_len - len(x)), 'constant', constant_values=(0,0)) for x in sources_list]
        # Read the mixture
        mixture, _ = sf.read(mixture_path, dtype="float32", start=start, stop=stop)
        # Convert to torch tensor
        mixture = torch.from_numpy(mixture)
        mixture = self.processor(mixture, sampling_rate=self.sample_rate).input_values[0]
        # Stack sources
